...
function carrerasAjax(id){
		...
		function agregarMaterias (contenido, listaCarreras){
			...
			for(let item of listaCarreras){
				...
				<tr>
				<td>
				<a href="/listarAulas/${item.id}" class="carousel-control-next boton-carreras" data-slide="next" onclick="ajaxMaterias(${item.id})"><h4 class="boton-carreras">${item.nombre}</h4></a>
				</td>
				<td>
